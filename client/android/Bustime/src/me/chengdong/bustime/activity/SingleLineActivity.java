...
public class SingleLineActivity extends BaseActivity {
	...
	TbSingleLineHandler tbSingleLineHandler = new TbSingleLineHandler(
			SingleLineActivity.this);
	...
	public void onClick(View v) {
		...
		case R.id.btn_favorite:
			...
			List<Line> lines = tbLineHandler.selectListByManyGuid("'"
					+ lineGuid + "'");
			if (lines.size() == 0) {
				break;
			}
	...
	private class QuerySingleLineTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			try {
				...
				List<SingleLine> localData = tbSingleLineHandler
						.selectList(lineGuid);
				if (localData.size() > 0) {
				...
				if (loadAllData) {
					result = DownloadData.getSingleLine(
							SingleLineActivity.this, lineGuid);
				} else {
					result = DownloadData.getRunSingleLine(
							SingleLineActivity.this, lineGuid);
				}
				if (result.success()) {
					if (loadAllData) {
						@SuppressWarnings("unchecked")
						List<SingleLine> temps = (List<SingleLine>) result
								.getData();
						mSingleLineList.clear();
						...
						@SuppressWarnings("unchecked")
						List<CodeValue> temps = (List<CodeValue>) result
								.getData();
						for (SingleLine singleLine : mSingleLineList) {
							for (CodeValue codeValue : temps) {
								if (codeValue.getCode().equals(
										singleLine.getStandCode())) {
					...
					// TODO 提示错误给用户
					TipUtil.tipDescription(SingleLineActivity.this,
							result.getMessage());
				}
